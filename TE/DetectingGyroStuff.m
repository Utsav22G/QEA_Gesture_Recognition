...
filtWZ = find_normal_acc(file1, 10, 1.0, 0.01, 0);
...
numberOfSpikes = detectTime(filtWZ)
function numberOfSpikes = detectTime(data)
    counter = 0;
    ...
    numberOfSpikes = counter;
end
function filtWZ = find_normal_acc(dataset, samp, freq_cutoff, attenuation, plotting)
    load(dataset);
